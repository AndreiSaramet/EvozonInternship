...
public class Game {
    ...
    public void background(final String childType) {
        System.out.printf("Once upon a time a %s named %s, aged %d, with %s hair who loved animals, entered an Animal Shelter with her father in order to adopt an animal.\n",
                childType, adopter.getName(), adopter.getAge(), adopter.getHairColour().toString().toLowerCase());
    }


    public void run() {
        final String childType = switch (adopter.getGender()) {
            case FEMALE -> "girl";
            case MALE -> "boy";
        };
        final String pronoun = switch (adopter.getGender()) {
            case FEMALE -> "she";
            case MALE -> "he";
        };
        ...
        background(childType);
        fellInLove(pronoun, animalType);
        dogState(animalType);
        adopt(childType);
        feed(childType, food);
        ...
        play(childType);
        visitVet(childType);
        changeFood(childType, food);
        dailyRoutine(childType, animalType);
    }

    private void dailyRoutine(String childType, String animalType) {
        System.out.printf("To make him slimmer, the %s had, almost daily, the following routine:\n%s\n", childType, String.join("\n", this.adopter.getRoutine(animalType)));
    }

    private void changeFood(String childType, DogFood food) {
        final SpecialFood specialFood = new SpecialFood("purina", 98.23, 412.31, LocalDate.of(2024, 12, 14), 3, "dry", "fish");
        System.out.printf("After the %s fed the dog with %s food for some time, he got %s, so she changed to %s for dogs which was %s in calories.\n", childType, food.getType(), dog.getWeight() >= 26 ? "fat" : "ok", specialFood.getClass().getSimpleName().toLowerCase(), specialFood.getCaloriesLevel());
    }

    private void visitVet(String childType) {
        System.out.printf("One day, the %s got him to the %s, which has %s him.\n", childType, vet.getClass().getSimpleName().toLowerCase(), String.join(" and ", vet.treatedDog()));
    ...
    private void play(String childType) {
        dog.play();
        dog.play();
        dog.play();
        dog.play();
    ...
    private void feed(String childType, AnimalFood food) {
        System.out.printf("As the dog was %s, the %s fed it with %s.\n", dog.getVibe(), childType, food.getClass().getSimpleName());
        dog.eat(food);
    }

    private void adopt(String childType) {
    ...
    private void fellInLove(String pronoun, String animalType) {
